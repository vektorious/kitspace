...
if (require.main !== module) {
  ...
  for (let folder of folders) {
    ...
    if (
      /^github.com/.test(info.id) &&
      (info.summary === '' || info.site == null)
    ) {
      const ghInfo = getGithubInfo(info.id) || {}
      if (info.summary === '') {
        if (ghInfo.description != null) {
          info.summary = ghInfo.description
        } else {
          console.warn(
            `WARNING: could not get GitHub description for ${folder}`
          )
        }
      }
      if (info.site == null) {
        if (ghInfo.homepage != null) {
          info.site = ghInfo.homepage
        } else {
          console.warn(`WARNING: could not get GitHub url for ${folder}`)
        }
      }
    }
    boards.push(info)
